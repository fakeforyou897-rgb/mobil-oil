...
<template>
  <article class="group relative min-w-0 overflow-hidden rounded-[22px] border border-white/[0.07] bg-[#14171C] p-2 transition duration-300 hover:-translate-y-1 hover:border-white/[0.14] hover:bg-[#181c22]">
    <RouterLink :to="`/product/${product.slug}`" class="relative block aspect-[1.06] overflow-hidden rounded-[17px] bg-[#20252C]">
    ...
    <div class="px-2 pb-1 pt-4">
      <div class="mb-1 flex items-start justify-between gap-2"><RouterLink :to="`/product/${product.slug}`" class="line-clamp-2 text-sm font-semibold leading-snug text-white hover:text-[#FF9145]">{{ product.name }}</RouterLink><span class="shrink-0 text-[10px] text-[#8E96A3]">{{ product.volume }}</span></div>
