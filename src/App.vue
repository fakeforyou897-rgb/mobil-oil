    ...
    <header class="sticky top-0 z-40 border-b border-white/[0.07] bg-[#0B0D10]/90 backdrop-blur-xl">
      <div class="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:h-[84px] lg:px-12">
        ...
        <nav class="hidden items-center gap-9 lg:flex">
          <RouterLink v-for="item in navItems" :key="item.name" :to="item.to" class="text-[13px] font-medium text-[#8E96A3] transition hover:text-white" :class="{ 'text-white': route.name === item.name }" :aria-current="route.name === item.name ? 'page' : undefined">{{ item.label }}</RouterLink>
        </nav>
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          ...
          <RouterLink to="/cart" aria-label="Cart" class="relative grid h-10 w-10 place-items-center rounded-full bg-[#FF6B00] text-[#0B0D10] transition hover:bg-[#ff8129]"><ShoppingBag :size="18" stroke-width="2.5" /><span v-if="cartLabel" role="status" aria-live="polite" class="absolute -right-1 -top-1 grid h-[18px] min-w-[18px] place-items-center rounded-full border-2 border-[#0B0D10] bg-[#58D5E6] px-1 text-[9px] font-bold">{{ cartLabel }}</span></RouterLink>
          <button aria-label="Open menu" aria-controls="mobile-navigation" :aria-expanded="mobileMenuOpen" class="grid h-10 w-10 place-items-center rounded-full text-[#8E96A3] sm:hidden" @click="mobileMenuOpen = !mobileMenuOpen"><X v-if="mobileMenuOpen" :size="20" /><Menu v-else :size="20" /></button>
        </div>
      </div>
      <div v-if="mobileMenuOpen" id="mobile-navigation" class="border-t border-white/[0.07] bg-[#101216] px-5 py-4 sm:hidden">
        <nav class="flex flex-col gap-1">
          <RouterLink v-for="item in navItems" :key="item.name" :to="item.to" class="rounded-xl px-3 py-3 text-sm text-[#8E96A3]" :class="{ 'bg-white/5 text-white': route.name === item.name }" :aria-current="route.name === item.name ? 'page' : undefined" @click="mobileMenuOpen = false">{{ item.label }}</RouterLink>
        </nav>
      </div>
    </header>
